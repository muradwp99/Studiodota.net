...
export async function generateMetadata(): Promise<Metadata> {
  const d = await getBlock("page.terms");
  return pageMetadata({ seo: d.seo, title: d.title, description: d.lede, path: "/terms" });
}

export default async function TermsPage() {
  const [d, site] = await Promise.all([getBlock("page.terms"), getBlock("site")]);
